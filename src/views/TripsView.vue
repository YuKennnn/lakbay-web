  ...
  <div class="pb-24 pt-10 min-h-screen bg-gray-50/30">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
        <h1 class="text-2xl sm:text-3xl font-bold text-[#2A8B8B]">Your Trips</h1>
        <div class="bg-white rounded-xl flex items-center px-4 py-2 border border-gray-200 shadow-sm w-full sm:w-72">
          <input type="text" placeholder="Search trips..." class="bg-transparent border-none outline-none w-full text-xs sm:text-sm text-gray-700">
        </div>
      </div>

      <div class="flex flex-col lg:flex-row justify-between items-start gap-6 sm:gap-8 mb-8 sm:mb-10 w-full">
        <div class="flex gap-2 bg-gray-100 p-1 rounded-xl w-full sm:w-auto overflow-x-auto no-scrollbar">
          <button v-for="tab in tabs" :key="tab" @click="activeTab = tab"
                  :class="['px-4 sm:px-6 py-2 rounded-lg text-xs sm:text-sm font-semibold transition whitespace-nowrap', 
                           activeTab === tab ? 'bg-white text-[#D97736] shadow-sm' : 'text-gray-500 hover:text-gray-700']">
            {{ tab }}
          </button>
        </div>
        <div class="bg-orange-50 border border-orange-100 rounded-xl p-3 sm:p-4 w-full lg:w-56 text-left lg:text-right shadow-sm flex lg:block justify-between items-center">
          <p class="text-[10px] sm:text-xs text-[#D97736] font-semibold uppercase tracking-wide mb-0 lg:mb-1">Total Spent</p>
          <p class="text-xl sm:text-2xl font-bold text-gray-800">{{ totalSpentDisplay }}</p>
        </div>
      </div>
      ...
      <div v-else class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 sm:gap-6">
        <div v-for="trip in filteredTrips" :key="trip.id" class="bg-white rounded-lg sm:rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md transition flex flex-col h-full group">
          ...
          <div class="p-2.5 sm:p-5 flex flex-grow flex-col justify-between">
            ...
            <div class="flex flex-col sm:flex-row gap-1.5 sm:gap-2 mt-1 sm:mt-2">
              <router-link :to="{ path: trip.link, query: { id: trip.id, status: trip.status } }" class="flex-grow bg-white border border-gray-200 py-1.5 sm:py-2.5 rounded-lg sm:rounded-xl text-[9px] sm:text-sm font-bold text-gray-600 hover:bg-gray-50 transition text-center">
              ...
              <button 
                ...
                class="bg-[#D97736]/10 text-[#D97736] px-3 py-1.5 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl text-[9px] sm:text-sm font-bold hover:bg-[#D97736]/20 transition flex items-center justify-center gap-1 sm:gap-2"
              >
